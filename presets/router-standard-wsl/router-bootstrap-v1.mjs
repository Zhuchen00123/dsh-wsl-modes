...
export function apply(ctx, config) {
  ...
  ctx.on('agent/inbox/inserted', (payload) => {
    ...
    if (!firstUserText.has(session.id) && text.trim()) {
      firstUserText.set(session.id, text.trim()) // capture BEFORE assembly
    }
    const target = agent
    if (target.inbox === undefined) return
    const mode = overrides.get(session.id) ?? firstUserText.get(session.id) ?? sessionMode(session)
  ...
  // rc.6 fix: session/event is not delivered to agent-plane presets; the
  // inbox claim happens BEFORE system-prompt/assemble, so capture the first
  // real user message here (issue #3/#10/#17).
  ...
  // while bootstrapping so they cannot disturb the RL anchor.
  ...
  registerTool({
    ...
    async execute(args) {
      ...
      const maxTokens = Number(args.maxTokens || 8192)
      let text = ''
      let reasoningText = ''
      ...
      // opencode-go flash sometimes puts the final answer in reasoning_content;
      // fall back to the reasoning tail so the caller still gets usable text.
      ...
      return `[mode-subagent ${bandFor(parsed)} | reasoning ${reasoningChars} chars${reasoningFallback ? ' | reasoning-fallback' : ''}]\n${head}${text.length > 3000 ? '\n…(truncated)' : ''}`
    },
  })
