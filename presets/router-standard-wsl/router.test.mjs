/** Router classifier + continuous mode tests (WSL/Flash-only edition). */
...
test('Flash-only: every model is treated as Flash (no Pro routing)', () => {
  assert.equal(isFlashModel('deepseek-v4-flash'), true)
  assert.equal(isFlashModel('deepseek-v4-pro'), true)
