...
export default class WslBashExecutor extends LocalBashExecutor {
    ...
    toWslPath(winPath) {
        ...
        try {
            ...
            if (out.length === 0)
                throw new Error('wslpath returned empty');
            if (this.pathCache.size >= 256)
    ...
    resolve(request) {
        ...
        for (const key of Object.keys(spec.dshEnv ?? {}))
            keys.add(key);
    ...
    classifyRunnerFailure(result) {
        if (result.exitCode === null || result.exitCode === 0)
            return false;
        return RUNNER_FAILURE_RULES.some((rule) => rule.fatalSignatures.some((sig) => result.stderr.text.toLowerCase().includes(sig.toLowerCase())));
    }
    async run(spec) {
